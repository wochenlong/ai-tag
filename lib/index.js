...
// 导入 Koishi 框架的上下文和模式对象
const koishi_1 = require("koishi");
// 定义插件的名称，用于在 Koishi 中注册和使用
exports.name = "ai-tag";
// 定义插件的用法说明，用于在 Koishi 中显示帮助信息
exports.usage = "这是随机tag插件，当有人在群聊中发送“tag”时触发。预设tag中不含任何nsfw元素，请放心使用。可以在`本地化 tag`中添加或修改你的随机tag，记得对照格式哦。欢迎来[论坛](https://forum.koishi.xyz/t/topic/740)提出你的好用tag~";
// 定义插件的配置模式，这里是一个对象模式
...
function apply(ctx) {
    ...
    ctx.command("tag", "随机输出绘画tag").action(({ session }) => {
        const message = session.text(ctx.config.prefix) +
            session.text(" ") +
            session.text("commands.tag.random-tag");
